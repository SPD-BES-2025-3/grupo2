...
class MQTTBrokerAdapter:
    def __init__(self, config: MQTTConfig):
        ...
        """
        self.config = config
        self.client = mqtt.Client(mqtt_enums.CallbackAPIVersion(2), client_id=config.client_id)
    ...
    def _on_connect(self, client, userdata, flags, rc, properties):
        ...
        if rc == 0:
            ...
            for topic in self.handlers.keys():
                ...
                logger.info(f"Tópico registrado automaticamente: {topic}")
        else:
            logger.error(f"Erro ao conectar ao broker MQTT, código: {rc}")

    def _on_message(self, client, userdata, msg):
        """Callback chamado quando uma mensagem é recebida."""
        logger.info(f"Nova mensagem recebida no tópico {msg.topic}")
        matched = False
        for pattern, handler in self.handlers.items():
            if self.matches_topic(pattern, msg.topic):
                try:
                    ...
                    logger.error(f"Erro ao enviar mensagem do MQTT para event loop: {e}")

        if not matched:
            logger.warning(f"Mensagem recebida sem handler relacionado no tópico {msg.topic}: {msg.payload.decode()}")
    ...
    def register_handler(self, topic: str, handler: Callable[[str, str], Coroutine[Any, Any, None]]):
        ...
        if self.client.is_connected():
            self.client.subscribe(topic)
            logger.info(f"Tópico {topic} assinado.")
    ...
    def stop(self):
        ...
        logger.info("Loop MQTT parado e cliente desconectado.")
